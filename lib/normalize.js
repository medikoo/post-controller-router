...
module.exports = function (conf/*, options*/) {
	...
	forEach(conf, function (conf, path) {
		var index, controller, validate, save;
		if (conf === true) conf = {};
		if (!isObject(conf)) {
			throw customError("Invalid configuration for " + stringify(path), 'INVALID_CONTROLLER');
		}
		controller = result[path] = {};
		if (conf.match !== undefined) {
			controller.match = conf.match;
			index = 1;
			while (conf['match' + (++index)] !== undefined) {
				controller['match' + index] = conf['match' + index];
			}
		}
